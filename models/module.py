...
from models.nn_ops import instance_norm, conv2d, deconv2d, lrelu
...
def discriminator(image, options, reuse=False, name="discriminator"):
    ...
    with tf.variable_scope(name):
        ...
        h1 = lrelu(instance_norm(conv2d(h0, options.df_dim*2, name='d_h1_conv'), name='d_bn1'))
        # h1 is (64 x 64 x self.df_dim*2)
        h2 = lrelu(instance_norm(conv2d(h1, options.df_dim*4, name='d_h2_conv'), name='d_bn2'))
        # h2 is (32x 32 x self.df_dim*4)
        h3 = lrelu(instance_norm(conv2d(h2, options.df_dim*8, s=1, name='d_h3_conv'), name='d_bn3'))
...
def generator_resnet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        def residule_block(x, dim, ks=3, s=1, in_name='res'):
            p = int((ks - 1) / 2)
            y = tf.pad(x, [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
            y = instance_norm(conv2d(y, dim, ks, s, padding='VALID', name=in_name + '_c1'), name=in_name + '_bn1')
            y = tf.pad(tf.nn.relu(y), [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
            y = instance_norm(conv2d(y, dim, ks, s, padding='VALID', name=in_name + '_c2'), name=in_name + '_bn2')
            return y + x
        ...
        r1 = residule_block(c3, options.gf_dim * 4, in_name='g_r1')
        r2 = residule_block(r1, options.gf_dim * 4, in_name='g_r2')
        r3 = residule_block(r2, options.gf_dim * 4, in_name='g_r3')
        r4 = residule_block(r3, options.gf_dim * 4, in_name='g_r4')
        r5 = residule_block(r4, options.gf_dim * 4, in_name='g_r5')
        r6 = residule_block(r5, options.gf_dim * 4, in_name='g_r6')
        r7 = residule_block(r6, options.gf_dim * 4, in_name='g_r7')
        r8 = residule_block(r7, options.gf_dim * 4, in_name='g_r8')
        r9 = residule_block(r8, options.gf_dim * 4, in_name='g_r9')
...
def generator_resnet_tiny(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        d1 = deconv2d(r4, options.gf_dim*2, 3, 2, name='g_d1_dc')
        d1 = tf.nn.relu(instance_norm(d1, 'g_d1_bn'))
        d2 = deconv2d(d1, options.gf_dim, 3, 2, name='g_d2_dc')
        d2 = tf.nn.relu(instance_norm(d2, 'g_d2_bn'))
        d2 = tf.pad(d2, [[0, 0], [3, 3], [3, 3], [0, 0]], "REFLECT")
...
def abs_criterion(inputs, target):
    return tf.reduce_mean(tf.abs(inputs - target))


def mae_criterion(inputs, target):
